...
int main(int argc, char **argv)
{
    ...
    start.target_pose.pose.position.x = 1.0;
    ...
    // Define a position and orientation for the robot to reach
    end.target_pose.pose.position.x = -5.0;
    ...
    // Check if the robot reached its goal
    if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
        ROS_INFO("Hooray, the base moved 5 meter backward, reached the endzone");
    else
        ROS_INFO("The base failed to move backward 5 meter for some reason");
    return 0;
}